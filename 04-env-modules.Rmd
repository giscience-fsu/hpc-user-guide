...
Everything which should be run on the nodes via Slurm requires to have the respective environment modules loaded in the Slurm template file whereas if you run something in RStudio server, the libraries installed in the underlying Ubuntu 20.04 container will be used.
```
...
Other than that, you should only need to load an R interpreter which is executing all scripts on the nodes.
The R interpreter in RStudio only live in the container and are not on the HPC nodes.

You should load the modules within your SLURM template.
