...
## Using RStudio Package Manager binaries

Instead of installing from source, you can speed up R package installations a lot by making use of RSPM binaries.
This is already configured by default in RSW.
To make use of them on the HPC, put the following into your `~/.Rprofile`:

```r
# otherwise RSPM does not detect CentOS7 OS
options(HTTPUserAgent = sprintf("R/%s R (%s)", getRversion(), paste(getRversion(), R.version$platform, R.version$arch, R.version$os)))
# set RSPM repo for HPC
if (any(grepl("CentOS-7", system("cat /etc/os-release", intern = TRUE)))) {
  options(repos = structure(c(CRAN = "https://packagemanager.rstudio.com/cran/__linux__/centos7/latest")))
}
```
